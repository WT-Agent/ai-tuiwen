  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">推文平台</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">种草目标</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">小红书种草与公众号爆款推文指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 小红书/公众号种草推文专家');
...
const metricsList = [
  {
    "key": "productSeedingConversion",
    "label": "产品种草转化力"
  },
  {
    "key": "headlineVisualAesthetic",
    "label": "标题视觉美感度"
  },
  {
    "key": "painpointResonanceIndex",
    "label": "痛点共鸣痛感指数"
  },
  {
    "key": "engagementIncentiveRating",
    "label": "互动引流吸引力"
  },
  {
    "key": "contentReadabilityPacing",
    "label": "内容阅读流畅节奏"
  }
];
const guideRules = [
  {
    "title": "用闺密口吻真实分享",
    "advice": "用“听我一句劝”、“踩坑无数总结出”等闺蜜分享语气，建立无防备的信任感",
    "avoid": "切忌使用生硬冷冰冰的硬广说明书调性"
  },
  {
    "title": "击中场景化具体痛点",
    "advice": "描述极具画面感的使用场景（如“熬夜加班后皮肤蜡黄敏感”），让读者对号入座",
    "avoid": "避免只讲成分名词而不讲对用户的具体好处"
  },
  {
    "title": "排版留白分段清晰",
    "advice": "短句为主，每段不超过 3 行，多用小标题分隔，提高手机端快速滑动浏览舒适度",
    "avoid": "严禁大段密密麻麻的文字堆叠在一起"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[TUIWEN_SCORES\][\s\S]*?\[\/TUIWEN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[TUIWEN_SCORES\](.*?)\[\/TUIWEN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('tuiwen_history_records');
...
const saveHistory = () => {
  localStorage.setItem('tuiwen_history_records', JSON.stringify(historyList.value));
};
